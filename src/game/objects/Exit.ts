...
export class ExitObject {
  ...
  constructor(x: number, y: number, scene: Phaser.Scene, exitConfig: ExitConfig) {
    const exitArea = new Phaser.GameObjects.Rectangle(scene, x - 50, y - 75, 100, 150, 0x078A7B)
    scene.add.existing(exitArea)
    this.exitArea = scene.physics.add.existing(exitArea) as Phaser.Types.Physics.Arcade.GameObjectWithStaticBody
    this.exitArea.body.pushable = false
    this.exitConfig = exitConfig
  }

  checkTruckDeparture: Phaser.Types.Physics.Arcade.ArcadePhysicsCallback = (e, t) => {
    const truck = t as Phaser.Types.Physics.Arcade.GameObjectWithBody & { x: number; y: number }
    const exitArea = e as Phaser.Types.Physics.Arcade.GameObjectWithBody & { x: number; y: number }
    const fullfilled: boolean = truck.getData('fullfilled')
    const truckId: boolean = truck.getData('id')

    const width = exitArea.body.width + 10
    const height = exitArea.body.height + 10

    const contained = Phaser.Geom.Rectangle.ContainsRect(
      new Phaser.Geom.Rectangle(exitArea.x - (width / 2), exitArea.y - (height / 2), width, height),
